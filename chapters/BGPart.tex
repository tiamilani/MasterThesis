...
As shown by the flows with a different color in \Cref{fig:AS_flow} a single
...
\begin{itemize}
	...
	\item[2] Route selection: This function is called at the end of the first
		phase, it collects all the eligible routes and evaluate them removing
...
information about the route (advertised or withdrawed).
...
and removing routes that has already been advertised to the pear.
...
\ac{MRAI} act as a timer which goal is to avoid continuous update storm caused
...
Without this option when a \ac{BGP} node discover a new best path to reach
...
The first one is caused by the protocol itself when it tryes to convergence 
on new knowledge.
...
new best paths that changes until the best possible path is taken in consideration.
...
For example the withdraw and the advertisement of a route at continuous interval.
...
Furthermore, other studies shows that the majority of the \ac{ADV} that travels
...
\Cref{fig:figure_of_merit} shows an hypnotic evolution of the \ac{RFD} filter,
...
Few years later the publication of the article from Pelsser et al. \cite{pelsser2011route}
...
a connection that physically pass over other devices.
...
\begin{figure}[ht]
     ...
     \end{subfigure}
		\caption{Differences between two levels of detail on the Internet graph
			\fxfatal{Remember to remove the dots behind the figures}}
...
behaviour of the network with certain events.
\begin{itemize}
...
The last type of topology used point to resemble the properties of the actual 
Internet graph.
