...
%\rlc{In questo capitolo ci sono ancora molti errori di forma, soggetti mancanti, frasi convolute e così via}
...
The protocol strictly depends on this parameter, because, as shown in \Cref{cha:bgp_fsm},
...
Using these graphs I will look for a possible interaction between \ac{MRAI} and other
...
The properties for the chosen environment are described in \Cref{tbl:internet_like_properties}.
...
I assume that every node knows in advance its own centrality value and that it
...
continuously grows linearly, as happened for the clique graph in \Cref{fig:clique_evolution}.
...
While, considering the time required to converge, in \Cref{fig:boxplot_internet_like_1000_time}
...
The graph chosen is the same used in \Cref{sec:bgp_mrai_strategy_dependance},
...
The trend in \Cref{fig:random_pareto_front} is similar to the one
generated from the same signal in \Cref{sec:bgp_mrai_strategy_dependance}.
...
\Cref{fig:pareto_comparison} present a comparison between the different
...
From \Cref{fig:pareto_comparison} is possible to see that all the strategies have the same
...
The evolution for the signal \q{AWA} is described in \Cref{fig:internt_like_1000_evolution_AWA}.
...
\Cref{fig:internt_like_1000_evolution_AWA}, compared to the plots in
\Cref{fig:internet_like_1000_constant_evolution,fig:internet_like_1000_dpc_evolution},
...
\ac{MRAI} doesn't affect the first message, the first \q{A}, but
it can affect the next two messages.
Some nodes can cache both the \q{WA} part of the signal and
...
The evolutions of the \q{AWAW} and \q{AWAWA} signals are shown in
...
different signal.
The results are presented in \Cref{fig:boxplot_internet_like_1000_allSignals}
...
In \Cref{fig:boxplot_internet_like_1000_allSignals} is possible to notice that
...
That's because, after the first \q{A}, giving an \ac{MRAI} timer long enough,
a node can compress a sequence that ends with another \q{A} to the
...
Firstly, I have decided to analyze multiple destinations, \num{10}
...
Those experiments use the environment described in \Cref{tbl:source_properties}.
...
As mentioned in \Cref{tbl:source_properties}, I decided to use another
...
The first and the last cases of \Cref{eq:dpc} are swapped, obtaining, as result, a
...
The plot in \Cref{fig:different_destinations_all} can be used to see the
...
In \Cref{fig:different_destinations_all} is possible to see that all the destinations
...
of the source node, deductible from the high spikes in \Cref{fig:different_destinations_all}.
...
\Cref{fig:different_levels_2,fig:different_levels_3}, is possible to notice that there
...
\Cref{fig:different_levels_4} is the worst-case scenario, not in
...
The results are shown in \Cref{fig:different_levels_comparison}.
