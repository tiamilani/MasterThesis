...
an Internet-like of \num{1000} nodes, in order to make the experiments comparable.
Thanks to this environment I'm going to run in total more than \num{600} complete
experiments.
For each \(MRAI_{mean}\) value, I will generate \num{10} different graphs with a random
...
In \Cref{fig:internt_like_1000_evolution_AWA} is possible to notice a big difference
...
the number of messages is less than \(\frac{1}{4}\) in respect of an \(MRAI_{mean}\)
of \SI{0}{\second}.
...
Those valley are provoked by the gain in performances that the compression gave
to the nodes, it overcomes the downside of having to make the neighbours wait longer.
The evolutions for the \q{AWAW} and \q{AWAWA} signals, are showed in
\Cref{fig:internt_like_1000_evolution_AWAW,fig:internt_like_1000_evolution_AWAWA}.

Like before, comparing the standard case with a \textit{fixed} \ac{MRAI} of, I
executed \num{100} different runs for each strategy and each different signal,
...
As first try, I have decided to analyze \num{10} different destinations, \num{10}
different nodes that shares a prefix, randomly chosen on the same graph, an
Internet like topology with \num{1000} nodes.
...
every strategy.
With these results is possible to analyze how different signal sources provoke
...
The first and the last cases of \Cref{eq:dpc} are inverted, having as result a
...
also the network performances change and evolve with different \ac{MRAI} values.
...
In \Cref{fig:different_destinations_all} is possible to see that all the \num{10} destinations
...
But, the choice of the strategy is more influent, the number of messages transmitted
...
are showed the performances of the nodes that use the \ac{DPC} strategy.
