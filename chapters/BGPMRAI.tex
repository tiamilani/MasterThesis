...
the incorrect use of it can lead to tremenodous consequences, even worst of not
having it at all.
...
\begin{figure}[h]
     ...
     \begin{subfigure}[b]{0.45\textwidth}
		 ...
		 \caption{Network perforcances \textbf{with} \textbf{\ac{IW}}}
     ...
     \begin{subfigure}[b]{0.45\textwidth}
		 ...
		 \caption{Network perforcances \textbf{withouth} \textbf{\ac{IW}}}
...
The effets of the first one are present in both the plots but in two different
moments.
...
time withouth obatining more useful information.
...
First of all \ac{MRAI} has a dependence on how it is setted, I'm going to compare
...
of the node that provoke the cahnge.

\section{Strategy dependence}
\label{sec:bgp_mrai_strategy_dependance}

Like I mentioned before, the network perfomances depends on the \ac{MRAI} strategies
choosen.
...
that study how the standard protcol evolves on an Internet environment.
...
The node that will execute the signal has been chosen randomly betwen 
...
assume that is precalculated and that every node knows it's own centrality to
set the timers.

To permit a comparison between thise two different strategies a constraint on the
\ac{MRAI} assignment has been introduced, the $mean$ of all the timers in the network
must be equal between the two strategies.
For the \textit{Fixed} strategy this is constraint intrisically respected.
...
\begin{figure}[h]
     ...
     \begin{subfigure}[b]{0.45\textwidth}
		 ...
		 \caption{Network perforcances, messages VS convergence time with different
			\ac{MRAI} values}
...
network delays and processing time. With, on the other hand an enormous ammount
of messages.
...
continuously grows linearly, as it happend for the clique graph in \Cref{fig:clique_evolution}.
This continious linear grow is dictated by the fact that nodes keep meaningfull
...
The pareto frontier is the set of values that are pareto efficent, this concept
has been already used in engineering to define the set of best outcomes from
the tradeoff of two different parameters \cite{goodarzi2014introduction}.
...
The results of the same environment withouth \ac{IW} are showed in 
...
For this reason points with the same \ac{MRAI} valure are comparable one another.
...
\fxfatal{Combersome, reading again it's not very clear what I'm explaining}
...
Both are equal at the begininngin with \ac{MRAI} equal \num{0} but after a while
both the number of transmitted messages and the convergence time diverge.
The number of messages with the \ac{DPC} strategy variates more and it converge
...
\cite{rfc4271} so I compared those strategies performances in a boxplot in 
...
the number of messages trasmitted by the \num{100} runs, we can see that
the two strategies, withouth \ac{IW}, are really close one another.
While in the time required for convergence, \Cref{fig:boxplot_internet_like_1000_time}
there are some huge difference between the two strategies, is not negligeable 
that with the \ac{DPC} strategy the time required is almost the double of the
standard time.

In cocnlusion we can say that the \ac{MRAI} strategy is one of the factor that 
can influence the Network perfomances. 
...
I would then study the space of possiblities that are generated through the 
pareto efficiency plot and compare the results with the pareto efficency
graphs.
...
In those experiments plays a role also the \q{\textit{readvertisement distribution}}
for the second and third \q{A}, it has been setted to a uniform distribution
between \SI{1}{\second} and \SI{60}{\second}, like the \q{\textit{withdraw distribution}}.

For those experiments I didn't evaluat the case with \ac{IW} deactivated.
\fxfatal{Explain why}

\fxfatal{all the plots has an \ac{MRAI} setep of \SI{10}{\second} to give a 
...
The \ac{DPC} strategy was abele to outcome the standard \textit{fixed} strategy
...
the number of messages is less than $1/4$ in respect of an \ac{MRAI} \texit{mean}
of \SI{0}{\second}.
The convergence time courve has a compleatly different trend in respect of the
...
compleatly avoid to send anything at all, because them have already transmitted
the first \q{A}.
The compleate compression of the signal \q{AWA} is \q{A}.
The other evolutions, for the \q{AWAW} and \q{AWAWA} signals are showed in 
...
\num{100} differnet runs for each strategy and each different signal, the results
...
restults in terms of messages transmitted, while it could have an higher 
convergence time.
...
The main ipothesis is that a node closer to the central clique, that generates
